...
export function getDbRows(columnArray, setTableRows) {
    ...
    const urlFile = `http://localhost/phpScripts/${fileName}`;
    axios.post(urlFile)
    .then(()  => {
        
        // Get Rows
        let tableName = columnArray[0][1].toLowerCase()
        const urlRow = `http://localhost/phpScripts/getTableRows.php?table=${tableName}`;
        axios.get(urlRow)
        .then(res  => {
...
export function setPageHeight() {
    function setMinHeight() {
        let navElem = document.getElementsByTagName("header")[0]
        let imageElem = document.getElementById("main-image")
        let bodyElem = document.getElementsByTagName("body")[0]

        // Make sure elements exist
        if((navElem !== null) && (imageElem !== null) && (bodyElem !== null)) {
            let navHeight = navElem.offsetHeight
            imageElem.style.minHeight = (document.documentElement.clientHeight - navHeight) + "px"
            bodyElem.style.height = (document.documentElement.clientHeight - 1) + "px"
        } 
    };
...
export function submitQuery (querySQL, setQuery, setQuerySQL, setQueryDisplay, getSqlDefault, setErrorMsg, setSuccessMsg, setTable, setColumnArray, setTableRows, resetPage) {
    ...
    const url = "http://localhost/phpScripts/dbMaintainExecuteQuery.php"
