...
const UserCheckoutPage = ({toggleLogin, showLogin}) => {
    ...
    useEffect(() => {
        const url = "http://localhost/phpScripts/fetchLocations.php"
        axios.get(url)
        .then(res => {
    ...
    const submitOrder = () => {
        if(user === null) {
            ...
            const url = "http://localhost/phpScripts/processUserPayment.php";
    ...
    const processCoupon = () => {
        setupCouponDatabase()
        const url = "http://localhost/phpScripts/processCoupon.php";
        let fdata = new FormData();
    ...
    function setupCouponDatabase() {
        let url = "http://localhost/phpScripts/createAndPopulateCoupons.php"
        let fdata = new FormData()
        axios.post(url, fdata)
        .catch((err) => {
            console.log(err)
